...
// ------------ Bootstrap
app.prepare().then(() => {
  // Custom request listener so we can serve health checks without routing through Next
  const requestListener = (req, res) => {
    const { pathname } = url.parse(req.url || '/', false);

    if (pathname === '/health' || pathname === '/readyz' || pathname === '/livez') {
      // Simple health endpoint for App Runner/ALB checks
      res.statusCode = 200;
  ...
  const io = new Server(httpServer, {
    ...
    // You can tune ping timeouts for load balancers if needed:
    // pingInterval: 25000,
    // pingTimeout: 60000,
    allowEIO3: true
  });
  ...
  const shutdown = (signal) => {
    ...
    stopPythonBackend();
    // Close Socket.IO & HTTP to stop accepting new connections
    io.close(() => {
      httpServer.close(() => {
