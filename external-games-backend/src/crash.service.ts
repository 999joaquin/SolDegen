...
export class CrashService {
  ...
  // TUNING for 60% loss / 40% win
  private readonly G = Math.log(2) / 8;  // Adjusted for more crashes
  private readonly MIN_CRASH = 1.01;     // Lower minimum = more early crashes = more losses
  ...
  private debit(userId: number, amount: number): void {
    this.balanceService.debit(userId, amount);
  }

  private credit(userId: number, amount: number): void {
    this.balanceService.credit(userId, amount);
  }
  ...
  private computeCrashRaw(roundId: string): number {
    ...
    const houseEdge = 0.60;              // 60:40 fairness (60% to players, 40% house edge)
  ...
  finishRound(): { roundId: string; crashMultiplier: number; durationMs: number } | null {
    ...
    const players = Array.from(r.players.values()).map(p => {
      ...
      const profit = cashed ? this.round2(p.cashedOut!.payout - p.bet) : this.round2(-p.bet);
      return {
        userId: p.userId,
        bet: p.bet,
        cashedOut: p.cashedOut ? { ...p.cashedOut } : undefined,
        profit, // NEW
      };
    });
    ...
    const item: CrashHistoryItem = {
      ...
      players, // use calculated array above
    };
