...
export class CrashGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  handleSendChatMessage(
    ...
    try {
      ...
      console.log(`Crash chat message from ${payload.username}: ${trimmedMessage}`);
    } catch (error) {
  ...
  async handleJoinRound(
    ...
    try {
      // Generate effective userId with fallback to socket-based ID
      const effectiveUserId = payload.userId ?? Number(Buffer.from(client.id).readUInt16BE(0) % 900000 + 100000);
      
      // Validate payload
      if (!payload.bet || !payload.clientSeed) {
      ...
      const result = this.crashService.joinOrCreate(effectiveUserId, payload.bet, payload.clientSeed);
  ...
  async handleCashout(
    ...
    try {
      ...
      const result = this.crashService.cashout(effectiveUserId);
